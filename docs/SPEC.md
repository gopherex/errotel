...
### Необязательные поля диагностики SDK

Envelope v1 допускает `exception.cause`, `exception.errors`, `exception.incomplete`
и верхнеуровневый `diagnostics`. Точные рекурсивные типы и проверки находятся в
protocol.ts и JSON Schema. Это совместимые необязательные поля по правилу §14;
смысл старых обязательных полей не меняется. `incomplete` явно обозначает цикл,
предел автоматического обхода или недоступное свойство. Ограничения обхода причины
не являются ограничением размера состояния. Политики очистки, фильтрации,
ограничения частоты и доставки описаны в [SDK guide](sdk.md).
